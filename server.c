...
int main (){
    ...
    {
        printf("\ninput how many client this server should accept: ");
        scanf(" %d", &client_count);
        if (client_count == max_client_count)
        {
            printf("cannot have more than %d clients. Ajusting amount down to 3", max_client_count);
            client_count = max_client_count;
        }
        printf("[SUCSESS] sock bound to address.\nListening for %d clients...\n",client_count);
    }




    int connections[max_client_count];
    int recv_size;
    int client_size;
    const char* hold    = "\nconnected to server, please wait to send messages...\n";
    const char* welcome = " welcome to kiki's server";
    ...
    while(1)
    {
        ...
        return_value = listen(server_socket, max_client_count);
        if(return_value != 0)
        {
        ...
        for(int x = 0; client_count > x; x++)
        {
            ...
            }else
            {
                ...
                return_value = send(client_socket, hold, strlen(hold), 0);
                printf("name: %d\n", return_value);
                if(return_value == SOCKET_ERROR )
                {
                    printf("[ERROR] failed to send welcome message to socket _%d_ returning: %d \nWSAGetLastError returned: %d ", client_socket, return_value, WSAGetLastError());
                    return -1;
                }
            }
        ...
        // print welcome messages to clients
        for(int x = 0; client_count > x; x++ ){
            return_value = send(client_socket, welcome, strlen(welcome), 0);
            if(return_value == SOCKET_ERROR )
            {
                printf("[ERROR] failed to send welcome message to socket _%d_ returning: %d \nWSAGetLastError returned: %d ", client_socket, return_value, WSAGetLastError());
                return -1;
